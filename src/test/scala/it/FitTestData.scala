...
import com.github.gradientgmm.optim.regularization._
...
object FitTestData{
  ...
  def run: Unit = {
    ...
    var model = GradientGaussianMixture(initialWeights,initialDists.clone,optim)

    if(batchSize.isDefined){
      model.setBatchSize(batchSize.get)
    }
    //var model = GradientGaussianMixture.fit(parsedData,k=3)
